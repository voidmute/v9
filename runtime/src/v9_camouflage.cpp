...
namespace
{
    ...
    auto default_log_dir() -> std::filesystem::path
    {
        ...
        if (size > 0 && size < std::size(buffer))
            return std::filesystem::path(buffer) / L"V9" / L"runtime";
        return std::filesystem::temp_directory_path() / L"V9" / L"runtime";
    }
    ...
    auto extract_embedded_bridge(const std::filesystem::path& log_dir, int port) -> std::filesystem::path
    {
        ...
        const auto bridge_path = native_dir / utf8_to_wide("v9bridge-" + hash + "-" + std::to_string(port) + ".dll");
        bool write = true;
